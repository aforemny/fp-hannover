module AutoSummary.Form.View exposing
    ( Model, State(..), idle
...
renderField customConfig ({ onChange, onBlur, disabled, showError } as fieldConfig) field =
    ...
    case field.state of
        ...
        Form.Summary fields ->
            fields
                |> List.map (maybeIgnoreChildError field.error >> renderField summaryConfig { fieldConfig | disabled = field.isDisabled || disabled })
                |> customConfig.section "Summary"
...
summaryConfig : CustomConfig msg (Html msg)
summaryConfig =
    { form = form
    , textField = inputFieldSummary "text"
    , emailField = inputField "email"
    , passwordField = inputField "password"
    , searchField = inputField "search"
    , textareaField = textareaField
...
inputFieldSummary : String -> TextFieldConfig msg -> Html msg
inputFieldSummary type_ { onChange, onBlur, disabled, value, error, showError, attributes } =
    Html.span
        []
        [ Html.text value ]
        |> withLabelAndError attributes.label showError error
